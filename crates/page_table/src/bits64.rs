...
impl<M: PagingMetaData, PTE: GenericPTE, IF: PagingIf> PageTable64<M, PTE, IF> {
    ...
    pub fn map_fault(
        ...
        {
            // *entry = Rv64PTE::new_fault_page(flags, page_size.is_huge());
            *entry = PTE::new_page(PhysAddr::from(0), MappingFlags::empty(), false);
        }
        #[cfg(target_arch = "aarch64")]
        {
            // *entry = GenericPTE::new_fault_page(flags, page_size.is_huge());
            *entry = PTE::new_page(PhysAddr::from(0), flags & !MappingFlags::READ, true);
        }
        #[cfg(not(any(
            target_arch = "riscv32",
            target_arch = "riscv64",
