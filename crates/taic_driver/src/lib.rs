...
impl Taic {
    ...
    pub fn add(&self, task_id: TaskId) {
        self.regs()
            .add()
            .write(|w| unsafe { w.bits(task_id.value() as _) });
    }
    ...
    pub fn switch_hypervisor(&self, hypervisor_id: TaskId) {
        self.regs()
            .switch_hypervisor()
            .write(|w| unsafe { w.bits(hypervisor_id.value() as _) });
    }
    ...
    pub fn switch_os<T: Sized>(&self, os_id: Option<TaskId>) {
        if os_id.is_none() {
            let current = unsafe {
                &mut *((self.regs().current().read().tcb().bits() << TASK_META_ALIGN)
                    as *mut TaskMeta<T>)
            };
            ...
            self.regs().switch_os().write(|w| unsafe { w.bits(0) });
            drop(lock);
        } else {
            self.regs()
                .switch_os()
                .write(|w| unsafe { w.bits(os_id.unwrap().value() as _) });
        }
    }
    ...
    pub fn switch_process<T: Sized>(&self, process_id: Option<TaskId>) {
        ...
        self.regs()
            .switch_process()
            .write(|w| unsafe { w.bits(process_id.map_or(0, |tid| tid.value()) as _) });
        drop(lock);
    }
    ...
    pub fn register_sender(
        ...
        self.regs()
            .register_send_task()
            .write(|w| unsafe { w.bits(send_task_id.value() as _) });
        self.regs()
            .register_send_target_os()
            .write(|w| unsafe { w.bits(recv_os_id.value() as _) });
        self.regs()
            .register_send_target_proc()
            .write(|w| unsafe { w.bits(recv_proc_id.value() as _) });
        self.regs()
            .register_send_target_task()
            .write(|w| unsafe { w.bits(recv_task_id.value() as _) });
    }
    ...
    pub fn register_receiver(
        ...
        self.regs()
            ...
            .write(|w| unsafe { w.bits(recv_task_id.value() as _) });
        self.regs()
            .register_recv_target_os()
            .write(|w| unsafe { w.bits(send_os_id.value() as _) });
        self.regs()
            .register_recv_target_proc()
            .write(|w| unsafe { w.bits(send_proc_id.value() as _) });
        self.regs()
            .register_recv_target_task()
            .write(|w| unsafe { w.bits(send_task_id.value() as _) });
    }
    ...
    pub fn remove_task(&self, task: TaskId) {
        self.regs()
            .remove()
            .write(|w| unsafe { w.bits(task.value() as _) });
    }
}
