...
impl<T> Pointable for T {
    ...
    /// 这里得到的是基于数据段的偏移量
    unsafe fn init(init: Self::Init) -> usize {
        let raw_ptr = Box::into_raw(Box::new(init)) as usize;
        raw_ptr - get_data_base()
    }

    unsafe fn deref<'a>(ptr: usize) -> &'a Self {
        &*((ptr + get_data_base()) as *const T)
    }

    unsafe fn deref_mut<'a>(ptr: usize) -> &'a mut Self {
        &mut *((ptr + get_data_base()) as *mut T)
    }

    unsafe fn drop(ptr: usize) {
        drop(Box::from_raw((ptr + get_data_base()) as *mut T));
    }
}
...
/// 一个拥有所有权的对分配对象，data 表示堆分配对象相对于数据段的偏移
...
impl<T: ?Sized + Pointable> Pointer<T> for Owned<T> {
    ...
    /// 根据 usize 来生成一个 Owned 实例，usize 为堆分配对象相对于数据段的偏移
    #[inline]
    unsafe fn from_usize(data: usize) -> Self {
...
impl<T> Owned<T> {
    ...
    pub unsafe fn from_raw(raw: *mut T) -> Owned<T> {
        ...
        Self::from_usize(raw - get_data_base())
    }
    ...
    pub fn into_box(self) -> Box<T> {
        ...
        mem::forget(self);
        unsafe { Box::from_raw((raw + get_data_base()) as *mut _) }
    }
