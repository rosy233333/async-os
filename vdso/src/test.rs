...
fn test_dyn_lib() {
    let mut cops_lib =
        std::fs::File::open("target/riscv64gc-unknown-linux-musl/release/libcops.so").unwrap();
    let mut elf_data = Vec::new();
    cops_lib.read_to_end(&mut elf_data).unwrap();
    ...
    println!("The elf base addr may be different in different arch!");
...
pub fn get_relocate_pairs(
    ...
    println!("Base addr for the elf: 0x{:x}", base_addr);
    if let Some(rela_dyn) = elf.find_section_by_name(".rela.dyn") {
        ...
        if let Some(dyn_sym_table) = elf.find_section_by_name(".dynsym") {
            ...
            for entry in data {
                ...
                let symbol_name = dyn_sym.get_name(elf).unwrap();
                println!(
                    "Relocating: symbol: {} symbol_value: 0x{:x}, addend: 0x{:x}",
                    symbol_name, symbol_value, addend
                );
