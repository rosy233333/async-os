...
/// * `argv` - *const usize
/// * `envp` - *const usize
pub async fn syscall_exec(args: [usize; 6]) -> SyscallResult {
    let path = args[0] as *const u8;
    let mut argv = args[1] as *const usize;
    let mut envp = args[2] as *const usize;
    let path = solve_path(AT_FDCWD, Some(path), false).await?;
...
/// * `size` - usize
pub async fn syscall_clone3(args: [usize; 6]) -> SyscallResult {
    let size = args[1];
        ...
        if clone_flags.contains(CloneFlags::CLONE_PIDFD) {
            unsafe {
                *(clone_args.pidfd as *mut u64) =
                    new_pidfd(new_task_id, OpenFlags::empty()).await? as u64;
            }
        }
        Ok(new_task_id as isize)
    } else {
...
/// * `option` - WaitFlags
pub async fn syscall_wait4(args: [usize; 6]) -> SyscallResult {
                ...
                match status {
                    ...
                    WaitStatus::Running => {
                        if option.contains(WaitFlags::WNOHANG) {
                            ...
                            match current_process().await.have_restart_signals().await {
...
pub async fn syscall_yield() -> SyscallResult {
    yield_now().await;
    Ok(0)
}
...
/// * `req` - *const TimeSecs
/// * `rem` - *mut TimeSecs
pub async fn syscall_sleep(args: [usize; 6]) -> SyscallResult {
    ...
    if current_process().await.have_signals().await.is_some() {
...
/// # Arguments
/// * `tid` - usize
pub async fn syscall_set_tid_address(args: [usize; 6]) -> SyscallResult {
    let tid = args[0];
...
pub async fn syscall_getpid() -> SyscallResult {
    Ok(current_process().await.pid() as isize)
}

/// To get the parent process id
pub async fn syscall_getppid() -> SyscallResult {
    Ok(current_process().await.get_parent() as isize)
}
...
pub async fn syscall_umask(args: [usize; 6]) -> SyscallResult {
    ...
    Ok(current_process().await.fd_manager.set_mask(new_mask) as isize)
}
...
/// * `option` - usize
/// * `arg2` - *mut u8
pub async fn syscall_prctl(args: [usize; 6]) -> SyscallResult {
