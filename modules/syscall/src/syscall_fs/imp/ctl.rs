...
use async_fs::api::{
    remove_dir, remove_file, rename, AsAny, AsyncFileIO, ConsoleWinSize, OpenFlags, Permissions, FIOCLEX, FIONBIO, TCGETS, TIOCGPGRP, TIOCGWINSZ, TIOCSPGRP
};
use async_io::Stream;
...
pub async fn syscall_getcwd(args: [usize; 6]) -> SyscallResult {
    let buf = args[0] as *mut u8;
    let len = args[1];
    debug!("Into syscall_getcwd. buf: {}, len: {}", buf as usize, len);
    let mut cwd = current_executor().get_cwd().await;
...
pub async fn syscall_mkdirat(args: [usize; 6]) -> SyscallResult {
    let dir_fd = args[0];
    let path = args[1] as *const u8;
    let mode = args[2] as u32;
    // info!("signal module: {:?}", process_inner.signal_module.keys());
    let path = solve_path(dir_fd, Some(path), true).await?;
    debug!(
    ...
    if async_fs::api::path_exists(path.path()).await {
        // 文件已存在
        return Err(SyscallError::EEXIST);
    }
    let _ = async_fs::api::create_dir(path.path());
    // 只要文件夹存在就返回0
    if async_fs::api::path_exists(path.path()).await {
        Ok(0)
    } else {
...
/// # Return
/// 成功执行:返回0。失败, 返回-1。
pub async fn syscall_chdir(args: [usize; 6]) -> SyscallResult {
...
pub async fn syscall_getdents64(args: [usize; 6]) -> SyscallResult {
    let fd = args[0];
    ...
    let mut dir_iter = async_fs::api::read_dir(path.path()).await.unwrap();
...
pub async fn syscall_renameat2(args: [usize; 6]) -> SyscallResult {
    let old_dirfd = args[0];
    let _old_path = args[1] as *const u8;
    let new_dirfd = args[2];
    let _new_path = args[3] as *const u8;
    let flags = args[4];
    let old_path = solve_path(old_dirfd, Some(_old_path), false).await?;
    axlog::error!("syscall_renameat2 old_path : {:?}", args);
    let new_path = solve_path(new_dirfd, Some(_new_path), false).await?;
    axlog::error!("syscall_renameat2 new_path: {:?}", args);

    let proc_path = FilePath::new("/proc").await.unwrap();
    ...
    if flags.contains(RenameFlags::NOREPLACE) {
        ...
        if async_fs::api::path_exists(new_path.path()).await {
    ...
    if !flags.contains(RenameFlags::EXCHANGE) {
        // 此时不是交换，而是移动，那么需要
        if async_fs::api::path_exists(new_path.path()).await {
            let old_metadata = async_fs::api::metadata(old_path.path()).await.unwrap();
            let new_metadata = async_fs::api::metadata(new_path.path()).await.unwrap();
    ...
    // 做实际重命名操作
    if !async_fs::api::path_exists(old_path.path()).await {
    ...
    if !flags.contains(RenameFlags::EXCHANGE) {
        ...
        if async_fs::api::path_exists(new_path.path()).await {
            ...
            if new_metadata.is_dir() {
                if let Err(err) = remove_dir(new_path.path()).await {
                    error!("error: {:?}", err);
                    return Err(SyscallError::EPERM);
                }
            } else if new_metadata.is_file() {
                if let Err(err) = remove_file(new_path.path()).await {
                    error!("error: {:?}", err);
                    return Err(SyscallError::EPERM);
                }
            }
        }
        if let Err(err) = rename(old_path.path(), new_path.path()).await {
            error!("error: {:?}", err);
            return Err(SyscallError::EPERM);
        }
    } else {
...
/// * `cmd`: usize
/// * `arg`: usize
pub async fn syscall_fcntl64(args: [usize; 6]) -> SyscallResult {
    let fd = args[0];
    let cmd = args[1];
    let arg = args[2];
    let process = current_executor();
    ...
    match Fcntl64Cmd::try_from(cmd) {
        ...
        Ok(Fcntl64Cmd::F_GETFD) => {
            if file.get_status().await.contains(OpenFlags::CLOEXEC) {
                Ok(1)
            } else {
                Ok(0)
            }
        ...
        Ok(Fcntl64Cmd::F_SETFD) => {
            if file.set_close_on_exec((arg & 1) != 0).await {
                Ok(0)
            } else {
        ...
        Ok(Fcntl64Cmd::F_GETFL) => Ok(file.get_status().await.bits() as isize),
...
/// * `request`: usize, 控制命令
/// * `argp`: *mut usize, 参数
pub async fn syscall_ioctl(args: [usize; 6]) -> SyscallResult {
    let fd = args[0];
    let request = args[1];
    let argp = args[2];
    let process = current_executor();
    ...
    if process.manual_alloc_for_lazy(argp.into()).await.is_err() {
    ...
    match request {
        ...
        FIONBIO => {
            ...
            if nonblock == 1 {
                let old_status = file.get_status().await;
                let _ = file.set_status(old_status | OpenFlags::NON_BLOCK).await;
            }
            return Ok(0);
        }
        FIOCLEX => Ok(0),
...
/// * `path`: *const u8, 文件的路径
/// * `mode`: usize, 文件的权限
pub async fn syscall_fchmodat(args: [usize; 6]) -> SyscallResult {
    let dir_fd = args[0];
    let path = args[1] as *const u8;
    let mode = args[2];
...
/// * `path`: *const u8, 文件的路径
/// * `mode`: usize, 文件的权限
pub async fn syscall_faccessat(args: [usize; 6]) -> SyscallResult {
    let dir_fd = args[0];
    let path = args[1] as *const u8;
    let mode = args[2];
    ...
    if let Ok(metadata) = async_fs::api::metadata(file_path.path()).await {
        if mode == 0 {
            //F_OK
            // 文件存在返回0,不存在返回-1
...
/// * `flags`: usize, 选项
pub async fn syscall_utimensat(args: [usize; 6]) -> SyscallResult {
    let dir_fd = args[0];
    let path = args[1] as *const u8;
    ...
    let (new_atime, new_mtime) = if times.is_null() {
        ...
        if process.manual_alloc_type_for_lazy(times).await.is_err() {
            return Err(SyscallError::EFAULT);
        }
    ...
    if (dir_fd as isize) > 0 {
        ...
        if let Some(file) = fd_table[dir_fd].as_ref() {
            if let Some(fat_file) = file.as_any().downcast_ref::<FileDesc>() {
                ...
                //     return ErrorNo::EPERM as isize;
                // }
                fat_file.stat.lock().await.atime.set_as_utime(&new_atime);
                fat_file.stat.lock().await.mtime.set_as_utime(&new_mtime);
            } else {
                return Err(SyscallError::EPERM);
            }
        }
        ...
        let file_path = solve_path(dir_fd, Some(path), false).await?;
        if !async_fs::api::path_exists(file_path.path()).await {
            if !async_fs::api::path_exists(file_path.dir().unwrap()).await {
                return Err(SyscallError::ENOTDIR);
            } else {
                return Err(SyscallError::ENOENT);
            }
