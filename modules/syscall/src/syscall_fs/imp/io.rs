...
pub async fn syscall_read(args: [usize; 6]) -> SyscallResult {
    let fd = args[0];
    ...
    let file = match process.fd_manager.fd_table.lock().await.get(fd) {
        Some(Some(f)) => f.clone(),
        _ => return Err(SyscallError::EBADF),
    };

    if file.get_type().await == FileIOType::DirDesc {
...
pub async fn syscall_write(args: [usize; 6]) -> SyscallResult {
    let fd = args[0];
...
pub async fn syscall_readv(args: [usize; 6]) -> SyscallResult {
    let fd = args[0];
    ...
    for i in 0..iov_cnt {
        ...
        match syscall_read(temp_args).await {
...
pub async fn syscall_pipe2(args: [usize; 6]) -> SyscallResult {
...
/// * flags: usize, 文件描述符标志
/// 返回值:成功执行,返回新的文件描述符。失败,返回-1。
pub async fn syscall_dup3(args: [usize; 6]) -> SyscallResult {
    let fd = args[0];
    let new_fd = args[1];
    let flags = args[2];
    let process = current_executor().await;
...
pub async fn syscall_openat(args: [usize; 6]) -> SyscallResult {
    let fd = args[0];
    ...
    // 分配 inode
    new_inode(path.path().to_string()).await.unwrap();
    // 如果是DIR
    info!("path: {:?}", path.path());
    if path.is_dir() {
        debug!("open dir");
        if let Ok(dir) = new_dir(path.path().to_string(), flags.into()).await {
...
/// * `fd`: usize, 要关闭的文件描述符。
/// 返回值:成功执行,返回0。失败,返回-1。
pub async fn syscall_close(args: [usize; 6]) -> SyscallResult {
    let fd = args[0];
...
/// * `count`: usize
/// * `offset`: usize
pub async fn syscall_pread64(args: [usize; 6]) -> SyscallResult {
    let fd = args[0];
    let buf = args[1] as *mut u8;
    let count = args[2];
    let offset = args[3];
    ...
    // let ret = file
    //     .seek(SeekFrom::Start(offset as u64)).await
    //     .and_then(|_| file.read(unsafe { core::slice::from_raw_parts_mut(buf, count) }).await);
    file.seek(SeekFrom::Start(old_offset)).await.unwrap();
...
/// * `count`: usize
/// * `offset`: usize
pub async fn syscall_pwrite64(args: [usize; 6]) -> SyscallResult {
    let fd = args[0];
    let buf = args[1] as *const u8;
    let count = args[2];
    let offset = args[3];
    ...
    // let ret = file.seek(SeekFrom::Start(offset as u64)).await.and_then(|_| {
...
/// * `offset`: *mut usize
/// * `count`: usize
pub async fn syscall_sendfile64(args: [usize; 6]) -> SyscallResult {
    let out_fd = args[0];
    let in_fd = args[1];
    ...
    if !offset.is_null() {
        ...
        Ok(out_file.write(buf.as_slice()).await.unwrap() as isize)
    } else {
...
/// * `buf`: *mut u8
/// * `bufsiz`: usize
pub async fn syscall_readlinkat(args: [usize; 6]) -> SyscallResult {
    let dir_fd = args[0];
    let path = args[1] as *const u8;
    ...
    if path.path() == "/proc/self/exe" {
        // 获取该进程符号链接对应的真正地址
        let file_real_path = process.get_file_path().await;
    ...
    if *path.path() != real_path(&(path.path().to_string())).await {
        // 说明链接存在
        let path = path.path();
...
/// * `offset`: isize
/// * `whence`: usize
pub async fn syscall_lseek(args: [usize; 6]) -> SyscallResult {
    let fd = args[0];
    ...
    if let Some(file) = fd_table[fd].as_ref() {
        ...
        let ans = if whence == 0 {
            ...
            // 即SEEK_END
            file.seek(SeekFrom::End(offset as i64)).await
        } else {
            return Err(SyscallError::EINVAL);
        };
...
/// * `len`: usize
/// * `flags`: usize
pub async fn syscall_copyfilerange(args: [usize; 6]) -> SyscallResult {
    let fd_in = args[0];
    ...
    let old_in_offset = in_file.seek(SeekFrom::Current(0)).await.unwrap();
    let old_out_offset = out_file.seek(SeekFrom::Current(0)).await.unwrap();
    ...
    let read_len = in_file.read(buf.as_mut_slice()).await.unwrap();
    // debug!("copy content: {:?}", &buf[..read_len]);

    let write_len = out_file.write(&buf[..read_len]).await.unwrap();
...
pub async fn syscall_ftruncate64(args: [usize; 6]) -> SyscallResult {
    ...
    if let Some(file) = fd_table[fd].as_ref() {
        if file.truncate(len).await.is_err() {
            return Err(SyscallError::EINVAL);
        }
    }
    Ok(0)
