...
impl DnsSocket {
    ...
    pub async fn new() -> Self {
        ...
        let handle = Some(SOCKET_SET.add(socket).await);
        Self { handle }
    }

    #[allow(dead_code)]
    /// Update the list of DNS servers, will replace all existing servers.
    pub async fn update_servers(self, servers: &[smoltcp::wire::IpAddress]) {
        SOCKET_SET
            .with_socket_mut::<dns::Socket, _, _>(self.handle.unwrap(), async |socket| {
    ...
    pub async fn query(&self, name: &str, query_type: DnsQueryType) -> AxResult<Vec<IpAddr>> {
        ...
        let query_handle = SOCKET_SET
            .with_socket_mut::<dns::Socket, _, _>(handle, async |socket| {
                socket.start_query(iface.lock().await.context(), name, query_type)
            })
            .await
            .map_err(|e| match e {
...
pub async fn dns_query(name: &str) -> AxResult<alloc::vec::Vec<IpAddr>> {
    let socket = DnsSocket::new().await;
    socket.query(name, DnsQueryType::A).await
}
