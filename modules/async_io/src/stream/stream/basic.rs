...
#[cfg(feature = "alloc")]
pub type BoxStream<'a, T> = Pin<alloc::boxed::Box<dyn AsyncStream<Item = T> + Send + 'a>>;

/// `BoxStream`, but without the `Send` requirement.
#[cfg(feature = "alloc")]
pub type LocalBoxStream<'a, T> = Pin<alloc::boxed::Box<dyn AsyncStream<Item = T> + 'a>>;
...
impl<S: ?Sized + AsyncStream + Unpin> AsyncStream for &mut S {
    type Item = S::Item;
...
{
    type Item = <P::Target as AsyncStream>::Item;
...
pub trait FusedStream: AsyncStream {
...
mod private_try_stream {
    ...
    impl<S, T, E> Sealed for S where S: ?Sized + AsyncStream<Item = Result<T, E>> {}
}

/// A convenience for streams that return `Result` values that includes
/// a variety of adapters tailored to such futures.
pub trait TryStream: AsyncStream + private_try_stream::Sealed {
...
where
    S: ?Sized + AsyncStream<Item = Result<T, E>>,
{
    type Ok = T;
    type Error = E;
...
mod if_alloc {
    ...
    impl<S: ?Sized + AsyncStream + Unpin> AsyncStream for Box<S> {
        type Item = S::Item;
