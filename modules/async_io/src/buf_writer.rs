...
///```
#[derive(Debug)]
pub struct IntoInnerError<W>(W, #[allow(dead_code)] io::Error);
    ...
    fn poll_flush_buf(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        ...
        while *this.written < len {
            match this.inner.as_mut().write(cx, &this.buf[*this.written..]) {
                ...
                Poll::Ready(Err(ref e)) if *e == io::Error::Interrupted => {}
                Poll::Ready(Err(e)) => {
                    ret = Err(e);
                    break;
                }
...
impl<W: AsyncWrite> AsyncWrite for BufWriter<W> {
    fn write(
        ...
        if buf.len() >= self.buf.capacity() {
            self.get_pin_mut().write(cx, buf)
        } else {
            Pin::new(&mut *self.project().buf).write(cx, buf)
        }
    }

    fn flush(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        ready!(self.as_mut().poll_flush_buf(cx))?;
        self.get_pin_mut().flush(cx)
    }

    fn close(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        ready!(self.as_mut().poll_flush_buf(cx))?;
        self.get_pin_mut().close(cx)
    }
}
