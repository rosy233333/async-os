...
async fn test_ramfs_ops(devfs: &RamFileSystem) -> VfsResult {
    ...
    assert_eq!(
        root.clone().lookup("f1/").await.err(),
        Some(VfsError::NotADirectory)
    );

    let node = root.lookup("////f1").await?;
    ...
    assert!(Arc::ptr_eq(
        &foo.clone().lookup("/f3").await?,
        &devfs.root_dir().await.lookup(".//./foo///f3").await?,
    ));
    assert_eq!(
        foo.clone()
            .lookup("/bar//f4")
...
async fn test_get_parent(devfs: &RamFileSystem) -> VfsResult {
    ...
    let node = root.clone().lookup("f1").await?;
    ...
    //     &devfs.root_dir().await.lookup(".//./foo/././bar/../..").await?,
    // ));
    // assert!(Arc::ptr_eq(
    //     &root.clone().lookup("././/foo//./../foo//bar///..//././").await?,
    //     &devfs.root_dir().await.lookup(".//./foo/").await?,
    // ));
    // assert!(Arc::ptr_eq(
    //     &root.clone().lookup("///foo//bar///../f3").await?,
    //     &root.lookup("foo/.//f3").await?,
    // ));
