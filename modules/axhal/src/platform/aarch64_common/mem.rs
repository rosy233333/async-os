...
pub(crate) fn platform_regions() -> impl Iterator<Item = MemRegion> {
    ...
    let iterator: Either<_, _> = if of::machin_name().unwrap().contains("raspi") {
        Left(
...
fn free_regions() -> impl Iterator<Item = MemRegion> {
    let iter = if let Some(nodes) = of::memory_nodes() {
        ...
        Left(filter_kernel_mem.flat_map(move |m| split_region(m, &fdt_region())))
    } else {
        Right(crate::mem::default_free_regions())
    };
